...
func Test_NewSourceObservableFile(t *testing.T) {
	...
	t.Run("error that may be raised when retrieving the file info", func(t *testing.T) {
		...
		src, err := newSourceObservableFile(path, DecoderFormatYAML, fs, &dFactory)
		switch {
		case src != nil:
			t.Error("returned a valid reference")
		case err == nil:
	...
	t.Run("error that may be raised when creating the decoder", func(t *testing.T) {
		...
		src, err := newSourceObservableFile(path, DecoderFormatUnknown, fs, &dFactory)
		switch {
		case src != nil:
			t.Error("returned a valid reference")
		case err == nil:
...
func Test_SourceObservableFile_Reload(t *testing.T) {
	t.Run("error if fail to retrieving the file info", func(t *testing.T) {
		...
		dFactory := decoderFactory{}
		_ = dFactory.Register(&decoderStrategyYAML{})
		src, _ := newSourceObservableFile(path, DecoderFormatYAML, fs, &dFactory)

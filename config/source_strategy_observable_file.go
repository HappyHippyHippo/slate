...
func newSourceStrategyObservableFile(fs afero.Fs, decoderFactory IDecoderFactory) (ISourceStrategy, error) {
	if fs == nil {
...
func (s sourceStrategyObservableFile) Create(args ...interface{}) (ISource, error) {
	...
	if path, ok := args[0].(string); !ok {
		...
		return NewSourceObservableFile(path, format, s.fs, s.decoderFactory)
	}
}

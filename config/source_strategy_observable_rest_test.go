...
func Test_NewSourceStrategyRestObservable(t *testing.T) {
	...
	t.Run("new observable rest source dFactory strategy", func(t *testing.T) {
		...
		sut, e := newSourceStrategyObservableRest(dFactory)
		switch {
		case e != nil:
			t.Errorf("returned the (%v) error", e)
		case sut == nil:
			t.Error("didn't returned a valid reference")
		case sut.(*sourceStrategyObservableRest).dFactory != dFactory:
			t.Error("didn't stored the decoder dFactory reference")
		default:
			client := sut.(*sourceStrategyObservableRest).cFactory()
			switch client.(type) {
			case *http.Client:
			default:
...
func Test_SourceStrategyRestObservable_Create(t *testing.T) {
	...
	t.Run("create the rest source", func(t *testing.T) {
		...
		sut, _ := newSourceStrategyObservableRest(dFactory)
		...
		sut.(*sourceStrategyObservableRest).cFactory = func() HTTPClient {
			return client
		}
...
func Test_SourceStrategyRestObservable_CreateFromConfig(t *testing.T) {
	...
	t.Run("missing uri", func(t *testing.T) {
		...
		case !errors.Is(e, err.ErrConfigPathNotFound):
			t.Errorf("returned the (%v) error when expecting (%v)", e, err.ErrConfigPathNotFound)
		}
	})
	...
	t.Run("create the rest source", func(t *testing.T) {
		...
		decoder.EXPECT().Close().Return(nil).Times(1)
		dFactory := NewMockDecoderFactory(ctrl)
		dFactory.EXPECT().Create(DecoderFormatJSON, gomock.Any()).Return(decoder, nil).Times(1)

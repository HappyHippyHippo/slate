...
func Test_config_AddSource(t *testing.T) {
	...
	t.Run("duplicate id", func(t *testing.T) {
		...
		if e := sut.AddSource("src", 0, src); e == nil {
			t.Error("didn't returned the expected error")
		} else if !errors.Is(e, serror.ErrDuplicateConfigSource) {
			t.Errorf("returned (%v) error when expecting (%v)", e, serror.ErrDuplicateConfigSource)
		}
	})
...
func Test_config_Source(t *testing.T) {
	t.Run("error if the source don't exists", func(t *testing.T) {
		...
		case e == nil:
			t.Error("didn't returned the expected error")
		case !errors.Is(e, serror.ErrConfigSourceNotFound):
			t.Errorf("returned (%v) error when expecting (%v)", e, serror.ErrConfigSourceNotFound)
		}
	})
...
func Test_config_SourcePriority(t *testing.T) {
	t.Run("error if the source was not found", func(t *testing.T) {
		...
		if e := sut.SourcePriority("invalid id", 0); e == nil {
			t.Error("didn't returned the expected error")
		} else if !errors.Is(e, serror.ErrConfigSourceNotFound) {
			t.Errorf("returned (%v) error when expecting (%v)", e, serror.ErrConfigSourceNotFound)
		}
	})

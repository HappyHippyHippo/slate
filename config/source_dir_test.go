...
func Test_NewSourceDir(t *testing.T) {
	...
	t.Run("error that may be raised when opening the dir", func(t *testing.T) {
		...
		sut, e := NewSourceDir(path, DecoderFormatYAML, true, fs, decoderFactory)
		switch {
		case sut != nil:
			t.Error("returned a valid reference")
		case e == nil:
	...
	t.Run("error opening the config file", func(t *testing.T) {
		...
		sut, e := NewSourceDir(path, DecoderFormatUnknown, true, fs, decoderFactory)
		switch {
		case sut != nil:
			t.Error("returned a valid reference")
		case e == nil:
	...
	t.Run("don't follow sub dirs if not recursive", func(t *testing.T) {
		...
		sut, e := NewSourceDir(path, DecoderFormatYAML, false, fs, decoderFactory)
		switch {
		case sut == nil:
			t.Error("didn't returned a valid reference")
		case e != nil:

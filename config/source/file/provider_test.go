...
func Test_Provider_Register(t *testing.T) {
	...
	t.Run("register components", func(t *testing.T) {
		...
		case !container.Has(ObsID):
			t.Errorf("didn't registered the observable strategy : %v", sut)
		}
	})
	...
	t.Run("retrieving observable strategy", func(t *testing.T) {
		container := slate.NewContainer()
		_ = (&fs.Provider{}).Register(container)
		_ = (&config.Provider{}).Register(container)
		_ = (&Provider{}).Register(container)
		...
		default:
			switch strategy.(type) {
			case *ObsSourceStrategy:
			default:
				t.Error("didn't return a observable strategy reference")
			}
		}
	})
}

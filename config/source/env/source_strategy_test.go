...
func Test_SourceStrategy_Accept(t *testing.T) {
	...
	t.Run("don't accept if type is missing", func(t *testing.T) {
		if (&SourceStrategy{}).Accept(&config.Config{}) {
			t.Error("returned true")
		}
	})

	t.Run("don't accept if type is not a string", func(t *testing.T) {
		if (&SourceStrategy{}).Accept(&config.Config{"type": 123}) {
			t.Error("returned true")
		}
	})

	t.Run("don't accept if type is not env", func(t *testing.T) {
		if (&SourceStrategy{}).Accept(&config.Config{"type": config.UnknownSourceType}) {
			t.Error("returned true")
		}
	})
}
...
func Test_SourceStrategy_Create(t *testing.T) {
	...
	t.Run("non-map mappings", func(t *testing.T) {
		src, e := (&SourceStrategy{}).Create(&config.Config{"mappings": 123})
		switch {
		case src != nil:
			t.Error("returned a valid reference")
		case e == nil:
	...
	t.Run("non-string key map mappings", func(t *testing.T) {
		src, e := (&SourceStrategy{}).Create(&config.Config{"mappings": config.Config{1: "value"}})
		switch {
		case src != nil:
			t.Error("returned a valid reference")
		case e == nil:
	...
	t.Run("non-string value map mappings", func(t *testing.T) {
		src, e := (&SourceStrategy{}).Create(&config.Config{"mappings": config.Config{"key": 1}})
		switch {
		case src != nil:
			t.Error("returned a valid reference")
		case e == nil:
	...
	t.Run("create the source", func(t *testing.T) {
		...
		path := "root"
		expected := config.Config{path: value}

		src, e := (&SourceStrategy{}).Create(&config.Config{"mappings": config.Config{env: path}})
		switch {
		case e != nil:
	...
	t.Run("no mappings on config", func(t *testing.T) {
		...
		src, e := (&SourceStrategy{}).Create(&config.Config{})
		switch {
		case e != nil:
			t.Errorf("returned the (%v) error", e)
		case src == nil:

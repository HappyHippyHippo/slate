...
func Test_Provider_Register(t *testing.T) {
	...
	t.Run("register components", func(t *testing.T) {
		...
		case !container.Has(ContainerFormatterStrategyJSONID):
			t.Error("didn't registered the slate formatter strategy json", e)
		case !container.Has(ContainerFormatterFactoryID):
			t.Error("didn't registered the slate formatter factory", e)
		case !container.Has(ContainerStreamStrategyConsoleID):
			t.Error("didn't registered the slate console stream strategy", e)
		case !container.Has(ContainerStreamStrategyFileID):
			t.Error("didn't registered the slate file stream strategy", e)
		case !container.Has(ContainerStreamStrategyRotatingFileID):
			t.Error("didn't registered the slate rotate file stream strategy", e)
		case !container.Has(ContainerStreamFactoryID):
			t.Error("didn't registered the slate stream factory", e)
		case !container.Has(ContainerID):
			t.Error("didn't registered the logger", e)
		case !container.Has(ContainerLoaderID):
			t.Error("didn't registered the slate loader", e)
		}
	})

	t.Run("retrieving slate formatter factory strategy json", func(t *testing.T) {
		container := slate.ServiceContainer{}

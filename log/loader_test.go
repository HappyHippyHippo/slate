...
func Test_Loader_Load(t *testing.T) {
	...
	t.Run("error creating stream", func(t *testing.T) {
		...
		streamFactory.EXPECT().CreateFromConfig(&partial).Return(nil, expected).Times(1)

		sut, _ := newLoader(manager, NewMockLogger(ctrl), streamFactory)
	...
	t.Run("error storing stream", func(t *testing.T) {
		...
		streamFactory := NewMockStreamFactory(ctrl)
		streamFactory.EXPECT().CreateFromConfig(&partial).Return(stream1, nil).Times(1)
		logger := NewMockLogger(ctrl)
		logger.EXPECT().AddStream("id", stream1).Return(expected).Times(1)

		sut, _ := newLoader(manager, logger, streamFactory)
	...
	t.Run("error on creating the reconfigured log streams", func(t *testing.T) {
		...
		stream1 := NewMockStream(ctrl)
		streamFactory := NewMockStreamFactory(ctrl)
		streamFactory.EXPECT().CreateFromConfig(&config1).Return(stream1, nil).Times(1)
		logger := NewMockLogger(ctrl)
		logger.EXPECT().AddStream("id", stream1).Return(nil).Times(1)
	...
	t.Run("error on storing the reconfigured log streams", func(t *testing.T) {
		...
		streamFactory := NewMockStreamFactory(ctrl)
		gomock.InOrder(
			streamFactory.EXPECT().CreateFromConfig(&config1).Return(stream1, nil),
			streamFactory.EXPECT().CreateFromConfig(&config1).Return(nil, expected),
		)
		logger := NewMockLogger(ctrl)
		logger.EXPECT().AddStream("id", stream1).Return(nil).Times(1)
	...
	t.Run("reconfigured log streams", func(t *testing.T) {
		...
		gomock.InOrder(
			streamFactory.EXPECT().CreateFromConfig(&config1).Return(stream1, nil),
			streamFactory.EXPECT().CreateFromConfig(&config2).Return(stream2, nil),
		)
		logger := NewMockLogger(ctrl)
		gomock.InOrder(

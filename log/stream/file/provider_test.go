...
func Test_Provider_Register(t *testing.T) {
	...
	t.Run("retrieving rotating strategy", func(t *testing.T) {
		container := slate.NewContainer()
		_ = (&fs.Provider{}).Register(container)
		_ = (&log.Provider{}).Register(container)
		_ = (&Provider{}).Register(container)

		strategy, e := container.Get(RotatingID)
		switch {
		case e != nil:
			t.Errorf("returned the unexpected error (%v)", e)
		case strategy == nil:
		...
		default:
			switch strategy.(type) {
			case *RotatingStreamStrategy:
			default:
				t.Error("didn't return a rotating strategy reference")
			}
		}
	})
}
